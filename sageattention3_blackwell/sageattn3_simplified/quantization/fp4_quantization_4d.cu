...
/*
 * WHAT THIS FILE DO:
 * Pre-process Q, K, V from FP16/BF16 to FP4 E2M1 format before the attention kernel.
 *
 * WHY PRE-QUANTIZE:
 * The attention kernel expects FP4 inputs with FP8 scale factors.
 * This kernel converts the BF16/FP16 inputs to that format.
 * Done once per forward pass before calling the attention kernel.
 *
 * FP4 E2M1 FORMAT:
 *   4 bits per value: 1 sign + 2 exponent + 1 mantissa.
 *   Representable values: {0, ±0.5, ±1, ±1.5, ±2, ±3, ±4, ±6}
 *   Max absolute value = 6.
 *   Two FP4 values packed per byte (8 bits total).
 *
 * BLOCK SCALING:
 *   Every 128 elements share one FP8 E4M3 scale factor.
 *   scale = max_abs_value / 6.0 (6 = max FP4 value)
 *   Quantized value = round_to_fp4(x / scale) = round_to_fp4(x * 6 / max_abs)
 *
 * FUNCTIONS:
 *
 * scaled_fp4_quant_kernel<head_dim, BLOCK_SIZE, permute, T>:
 *   Main quantization kernel. Input: [B, N, D] or [B, L, N, D].
 *   Each thread handles 128 consecutive elements (one scale factor block).
 *   1. Load 128 values from global memory (vectorized 128-bit loads)
 *   2. Find max abs value across 128 elements (warp reduction)
 *   3. Compute scale = max_abs / 6.0
 *   4. Store scale as FP8 E4M3
 *   5. Quantize each value: q = round_to_fp4(x / scale)
 *   6. Pack 2 FP4 values per byte, store to output
 *
 * scaled_fp4_quant_trans_kernel:
 *   Same but transposes V from [B, L, N, D] -> [B, N, D, L] (or [B, D, N, L]).
 *   Transposed format needed for P*V^T matrix multiply in attention kernel.
 *   Uses shared memory tiling for coalesced access during transpose.
 *
 * PERMUTE MODE (permute=true):
 *   Reorders elements in Q and K to match the memory layout expected by
 *   blockscaled MMA hardware. MMA expects specific interleaving of FP4 values
 *   within each 32-bit register. Permutation determined by hardware constraints.
 *
 * Python exports (via pybind11):
 *   fp4quant_cuda.scaled_fp4_quant(input, ...) -> (fp4_output, scale_output)
 *   fp4quant_cuda.scaled_fp4_quant_permute(input, ...) -> (fp4_output, scale_output)
 *   fp4quant_cuda.scaled_fp4_quant_trans(input, ...) -> (fp4_output_transposed, scale_output)
 */
