...
/*
 * WHAT THIS FILE DO:
 * Write the output O tensor from shared memory to global memory.
 *
 * TWO WRITE PATHS:
 *
 * 1. mma_store() [called by Consumer warp groups]:
 *    Convert O from float32 accumulators to BF16/FP16.
 *    Write to shared memory O buffer (smem_o) using STSM instruction.
 *    STSM = "Store to Shared Memory" - warp-level async store.
 *    Uses make_tiled_copy_C to map MMA output layout to SMEM layout.
 *    After this: smem_o contains the BF16/FP16 output tile.
 *
 * 2. tma_store() [called by Epilogue Producer warp, warp 1 of producer group]:
 *    TMA copy from smem_o to global memory.
 *    Only one thread issues the TMA store (elect_one_sync equivalent).
 *    tma_store_arrive(): signal TMA copy started.
 *    store_tail() = tma_store_wait<0>(): wait for all pending TMA stores to complete.
 *
 * WHY TWO STEPS:
 * Consumer warp group has too many threads to efficiently drive TMA store.
 * Instead: consumer writes to SMEM (fast), then dedicated epilogue warp issues TMA.
 * This separates MMA compute from output memory traffic.
 *
 * store_zero():
 *    Used when causal attention makes Q tile have no valid K tokens.
 *    Write zeros to O and +inf to LSE (log-sum-exp) directly without going through SMEM.
 *    Uses vectorized global memory store (128-bit loads, kGmemElemsPerLoad=8 for BF16).
 *
 * SMEM O LAYOUT (SmemLayoutO):
 *    Uses ss_smem_selector for GMMA-K-major swizzling.
 *    Swizzle = rearrange bytes in SMEM to avoid bank conflicts when loading.
 *    "as_position_independent_swizzle_tensor" = apply swizzle relative to start of buffer.
 *
 * NUMERIC CONVERSION:
 *    cutlass::NumericArrayConverter<BF16, float, N>: convert N float32 -> N BF16 at once.
 *    Applied to entire O fragment before STSM store.
 */
