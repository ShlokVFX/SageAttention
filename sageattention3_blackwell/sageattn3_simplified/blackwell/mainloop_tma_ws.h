...
/*
 * WHAT THIS FILE DO:
 * Main attention computation loop: TMA load Q/K/V + Q*K^T MMA + softmax + P*V MMA.
 *
 * TWO KEY FUNCTIONS:
 *
 * === load() [called by Producer warp group, warp 0] ===
 * Load Q once, then loop over K/V blocks:
 *   1. producer_acquire: get a free SMEM pipeline slot (block if full)
 *   2. TMA copy GMEM -> SMEM (async, 1 thread issues, hardware does transfer)
 *   3. advance pipeline write pointer (signal "slot filling")
 * Consumer waits on pipeline barrier. TMA decrements barrier when done.
 *
 * HOW TMA COPY WORK:
 *   copy(tma.with(barrier, mcast_mask), src_gmem, dst_smem)
 *   "with(barrier)" = decrement this barrier when transfer done
 *   Consumer does consumer_wait(pipeline, state) to block until barrier = 0
 *   transaction_bytes = bytes expected per slot (barrier knows when all arrived)
 *
 * === mma() [called by Consumer warp groups] ===
 * For each tile this consumer handles:
 *   1. Wait Q ready -> SMEM->RMEM copy Q + SFQ -> release Q slot
 *   2. Initialize S accumulator (QK product), O accumulator (output)
 *   3. For n_block = n_block_max-1 down to 0:
 *      a. Wait K slot ready
 *      b. SMEM->RMEM copy K + SFK for first k_block
 *      c. Set S = delta_s (initial value for smooth quant correction)
 *      d. MMA: S += Q * K^T (one k_block at a time, pipeline copy+mma)
 *      e. Apply causal mask (set future positions to -inf)
 *      f. Online softmax + FP4 quantize S -> P (see softmax_fused.h)
 *      g. quantize(): float P -> FP4 E2M1 packed uint32 + FP8 scale
 *      h. Wait V slot ready, copy Vt + SFVt SMEM->RMEM
 *      i. MMA: O += P * Vt (FP4 block-scaled)
 *      j. rescale_o(): multiply old O by correction (online softmax scale update)
 *      k. Release K and V pipeline slots
 *   4. softmax.finalize(): divide O by accumulated softmax sum
 *
 * CuTe TENSOR NAMING:
 *   mX = global memory tensor   (mQ, mK, mVt)    -- full GMEM view
 *   gX = global tile tensor     (gQ, gK, gVt)    -- tile sliced from GMEM
 *   sX = shared memory tensor   (sQ, sK, sVt)    -- SMEM buffer
 *   rX = register fragment      (tSrQ, tOrVt)    -- in registers
 *   tXgY = TMA copy src/dst     (tQgQ = Q TMA source)
 *   tXsY = TMA copy dst/src     (tQsQ = Q TMA dest in SMEM)
 *
 * PREFIX MEANING:
 *   Q = "related to Q loading"  (tQgQ, tQsQ, tQgSFQ)
 *   K = "related to K loading"  (tKgK, tKsK, tKgSFK)
 *   V = "related to V loading"  (tVgVt, tVsVt)
 *   S = "related to QK gemm"    (tSrQ, tSrK, tSrS)
 *   O = "related to PV gemm"    (tOrVt, tOrO)
 *
 * DELTA_S (smooth quantization):
 *   When quantizing Q to FP4, subtract per-block mean to reduce error.
 *   delta_s[i] = correction term for each K column block.
 *   tSrS initialized to delta_s, then MMA accumulates: S += Q_fp4 * K_fp4
 *   Result = Q_fp4*K_fp4 + delta_s ≈ original Q * K (corrected for quant error)
 *
 * CAUSAL MASK:
 *   Mask (row, col) if col >= row + 1 + seqlen_k - n_block*kBlockN - seqlen_q + m_block*kBlockM
 *   Set masked elements to -inf -> softmax gives 0 weight (ignored in output)
 *
 * QUANTIZE LAMBDA:
 *   After softmax, P in float32. Convert to FP4 E2M1 + FP8 UE4M3 scale.
 *   packed_float_to_e2m1(): 8 floats -> 1 uint32 (8 FP4 packed)
 *   packed_float_to_ue4m3(): 4 floats -> 1 uint32 (4 FP8 packed)
 *   __shfl_xor_sync: exchange scale factor with neighbor thread to pack correctly
 *   (Hardware needs scale in specific layout - 2 threads share 1 scale register)
 */
