...
/*
 * WHAT THIS FILE DO:
 * C++ binding between Python (PyTorch) and the CUDA attention kernel.
 * Called from Python as: fp4attn_cuda.mha_fwd(q, k, v, ...)
 *
 * set_params_fprop():
 *   Fill Flash_fwd_params struct from PyTorch tensors.
 *   Reads tensor data pointers, strides, shapes from PyTorch.
 *   Compute strides: how many elements to jump per token/head/batch.
 *   scale_softmax_log2 = (1/sqrt(d)) * log2(e)
 *     = softmax temperature * log2(e) for exp2 trick in softmax.
 *   Called once per forward pass, very cheap.
 *
 * run_mha_fwd():
 *   Pick correct kernel variant based on dtype and headdim.
 *   Currently: FP16/BF16 output, headdim 64 or 128.
 *   Calls run_mha_fwd_<T, Headdim>() which instantiates correct kernel traits.
 *
 * mha_fwd():
 *   Main function called from Python.
 *   Validates inputs (shape, dtype, GPU, contiguous).
 *   Allocates output tensor O.
 *   Fills params struct.
 *   Launches kernel.
 *   Returns O tensor.
 *
 * PYBIND11:
 *   PYBIND11_MODULE(fp4attn_cuda, m) = register Python module named "fp4attn_cuda".
 *   m.def("mha_fwd", ...) = expose mha_fwd() function to Python.
 *   After build: import fp4attn_cuda; fp4attn_cuda.mha_fwd(q, k, v, ...)
 */
